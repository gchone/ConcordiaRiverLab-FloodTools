...
# v1.1 - Mai 2020 -  Ménage pour ajout dans le package FloodTools. Légère modif du smoothing (débug)
...
def execute_WSsmoothing(r_flowdir, str_frompoint, r_z, r_zerr, str_zwssm, smooth_perc = 0.9):
    ...
    for tree in trees:
        ...
        for segment, prev_cs, cs in tree.browsepts():
            if prev_cs != None:
                ...
                cs.zsmooth2 = max(((1 - smooth_perc ** (cs.dist / 10.)) * cs.ztosmooth + smooth_perc ** (cs.dist / 10.) * prev_cs.zsmooth2), cs.z_breach)
        ...
        for segment, prev_cs, cs in tree.uptodown_browsepts():
            if prev_cs != None:
                if hasattr(cs, "zsmooth1"):
                    ...
                    cs.zsmooth1 = min((
                        (1 - smooth_perc ** (cs.dist / 10.)) * cs.ztosmooth + smooth_perc ** (cs.dist / 10.) *
                    ...
                    cs.zsmooth1 = min(((1 - smooth_perc ** (cs.dist / 10.)) * cs.ztosmooth + smooth_perc ** (cs.dist / 10.) * prev_cs.zsmooth1), cs.z_fill)
            else:
