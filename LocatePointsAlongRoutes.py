...
import arcpy
def execute_LocatePointsAlongRoutes (points_p, shapefile_A, output, distance_param)
...
# The point and routes shapefiles (or feature classes within a GDB) are saved as temporary layers.
    ...
    arcpy.MakeFeatureLayer_management(points_p, "pts_layer")
    arcpy.MakeFeatureLayer_management(shapefile_A, "route_layer")


    cursor = arcpy.da.SearchCursor(shapefile_A, ["RID"])
    list_tables = []
    ...
    for reach in cursor:
        ...
        arcpy.LocateFeaturesAlongRoutes_lr("pts_layer", "route_layer", "RID", distance, table)
