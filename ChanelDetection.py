...
def execute_ChanelDetection(r_streams, r_dem, niter, offlim, brch, postpro, checkelev, localtol, globaltol, watsurf, messages):
    env.snapRaster = r_streams
    env.extent = r_streams
    niter = max(niter, 1)
    maskras = None

    if offlim and offlim != "#":
        maskras = Con(IsNull(offlim), 0)

    newstreams = Con(IsNull(r_streams), r_streams, 0) # replace channel values by 0

    if brch and brch != "#":
        brchras = CreateScratchName("det", data_type="RasterDataset", workspace="in_memory")
        PolylineToRaster(brch, "OBJECTID", brchras, "MAXIMUM_LENGTH", cellsize=r_dem)
        newstreams = Con(IsNull(brchras), newstreams, 0)

    growthcheck = False
    for ii in range(0, niter, 1):
        localmax = FocalStatistics((r_dem + newstreams), NbrRectangle(3, 3, "CELL"), "MAXIMUM", "DATA")

        newstreams = Con((r_dem - localmax) <= globaltol, 0)
        ...
        del localmax
        if growthcheck:
            checkras = Con(IsNull(newstreams), 0, 1) - oldgrowth
        ...
        if ((ii+1) % 10) == 0:  # Test si le raster des sources a évolué à chaque 10 nouvelles itérations
            oldgrowth = Con(IsNull(newstreams), 0, 1)
            growthcheck = True
    ...
    if postpro:
        ...
        for ii in range(0, niter, 1):
            nbneighbors = FocalStatistics(newstreams, NbrRectangle(3, 3, "CELL"), "SUM", "DATA")
            if checkelev:
                nottoohigh = r_dem - FocalStatistics(streamelev, NbrRectangle(5, 5, "CELL"), "MEAN", "DATA")
                newstreams = Con((nbneighbors >= 6) & (nottoohigh <= localtol), 1, newstreams)
                del nbneighbors, nottoohigh
            else:
                newstreams = Con(nbneighbors >= 6, 1, newstreams)
                del nbneighbors
    ...
    sources = Con(IsNull(newstreams), r_streams, r_dem)
